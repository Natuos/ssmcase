    ...
    public String uIndex(Model model, Item item,HttpServletRequest request){
        ...
        if(!CollectionUtils.isEmpty(fatherList)){
            ...
            for(ItemCategory ic:fatherList){
                ...
                //将遍历的一级类目元素赋值给创建的类的对应属性
                dto.setFather(ic);
                //查询二级类目
                //根据之前查询的一级类目以其id值作为二级类目的父id查询条件
                ...
                //将查询的二级类目赋值给创建的类的对应属性
                dto.setChildren(children);
                //将获得的dto对象加入列表中（包含一个一级类目和它对应的所有的二级类目）
                list.add(dto);
                //将列表传入前端页面
        ...
        //折扣商品
        List<Item> zks = itemService.listBySqlReturnEntity("select * from item where isDelete=0 and zk is not null order by zk limit 0,10");
